...
function colorpicker(showid,fun,id2)
{
	...
	colorTable='<style>a.close-own{background:url(cross.png) no-repeat left 3px;display:block;width:16px;height:16px;position:absolute;outline:none;right:7px;top:8px;text-indent:200px; overflow: hidden}a.close-own:hover{background-position: left -46px}</style>'
			   ...
			   +'<td width="3"><td><input type="text" name="HexColor" size="7" id="input_colorId" style="border:inset 1px;font-family:Arial;" value="#000000"></td><td>&nbsp;&nbsp;<a href="javascript:;" onclick="enter_title();" title="确认自定义颜色">[确定]</a>&nbsp;<a href="javascript:;" onclick="clear_title();" title="清除设定的标题颜色">[清除]</a>' +
