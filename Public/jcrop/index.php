...
$(function(){
	//document.frames("frame").location.reload(true);
	...
	$("#SaveBtn").click(function(){
		...
		$.ajax({
			url : "/Public/jcrop/jcrop.php?imgurl=<?php echo $imgurl; ?>&x1="+ x1 +"&y1="+ y1 +"&x2="+ x2 +"&y2="+ y2 +"&iw="+ iw +"&ih=" + ih +"&wm=" + wm,
			type:'get',
			dataType:'json',
			beforeSend:function(){
			},
...
function GetReturn(data, textStatus, xmlHttp)
{
	if(data.status== true)
	{
		//alert(window.parent.document.getElementById("slt").value);
		//window.parent.document.getElementById("slt").value=data.message;
		$(window.parent.document.getElementById("slt")).val(data.message);
		$(window.parent.document.getElementById("slturl")).attr('src',data.message+"?code"+Math.random());
		//$("body,html").parent().parent().find("#slt").attr("sx",data.message);
		CloseJcrop();
	}
	else
	{
