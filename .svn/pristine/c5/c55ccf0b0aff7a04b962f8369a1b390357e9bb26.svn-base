...
class BannerController extends Controller {
	...
	public function delete(){
        ...
        $this->success('删除成功',U('Banner/lis',array('cid'=>$cid)),1);
    }
    public function lis(){
		$bmodel=M('Banner');
		$list=$bmodel->select();
    ...
    public function sc()
    {
        ...
        if ($result) {
            $this->success('添加成功', U('Banner/lis'), 1);
        } else {
            $this->error('添加失败');
        }
    }
	...
	public function ajax_edit(){
	    ...
	    if($result!==false){
	        $this->success('图片修改成功',U('Banner/edit',array('id'=>$id),1));
	        exit;
	    }else{
	...
	public function add_pic(){
		...
		if($result!==false){
			$this->success('上传成功',U('Banner/edit',array('id'=>$id,'p'=>$p,'cid'=>$cid)),1);
			exit;
        }else{
			$this->error($model->getError());
        }        		
