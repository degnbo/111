        ...
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                                <span class="">
                                    <a href="<?php echo U('Order/lis',array('p'=>I('get.p')));?>" class="btn btn-primary ">订单列表
                                    </a>
                                </span>
                </div>
                <div class="ibox-content">
                    <table class="table table-striped table-bordered table-hover dataTables-example">
                        ...
                        <tr>
                            <td>logo图标:</td>
                            <td>
                                <?php if($list["pic"] != ''): ?><img src="/Public/Uploads/<?php echo ($list["pic"]); ?>" width="100px" height="80px"/>
                                    <?php else: ?>
                                    --<?php endif; ?>
                            </td>
                        </tr>
                        <tr>
                            <td>活动开始时间:</td>
                            <td>
                                <?php if(time() >= $list['start_time']): echo date("Y年m月d日 H:i:s",$list['start_time']);?>(已开始)
                                    <?php else: ?>
                                    <?php echo date("Y年m月d日 H:i:s",$list['start_time']);?>(未开始)<?php endif; ?>
                            </td>
                        </tr>
                        <tr>
                            <td>报名人:</td>
                        ...
                        <tr>
                            <td>支付状态:</td>
                            <td>
                                <?php if($list["pay_status"] == 0): ?>待支付
                                <?php elseif($list["pay_status"] == 1 ): ?>
                                已支付
                                （<a onclick="return_money('<?php echo ($list["id"]); ?>')" href="javascript:void(0);">手动退款</a>）
                                <?php elseif($list["pay_status"] == 2 ): ?>
                                退款中
                                <?php elseif($list["pay_status"] == 3 ): ?>
                                签到，已退款
                                <?php elseif($list["pay_status"] == 4 ): ?>
                                活动取消
                                <?php elseif($list["pay_status"] == 5 ): ?>
                                非签到，已退款
                                <?php elseif($list["pay_status"] == 6 ): ?>
                                未退款<?php endif; ?>
                            </td>
                        </tr>
                        <script>
                            function return_money(id){
                                $.ajax({
                                    url:"<?php echo U('Order/join_return');?>",
                                    data:{id:id},
                                    dataType:'json',
                                    type:'get',
                        ...
                        <tr>
                            <td>活动举报消息:</td>
                            <td>
                                <?php if($alist1 != ''): ?><p>发布者<span style="color: green">(<?php echo ($list["pname"]); ?>)</span> 举报内容：<span style="color: green"><?php echo ($alist1["content"]); ?></span>
