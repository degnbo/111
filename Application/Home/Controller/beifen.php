...
class IndexController extends CommonController
{
    ...
    public function set_notice($phone, $code)
    {
        $redis = new \redis();
        $result = $redis->connect('127.0.0.1', 6379);
        $data = json_encode(array('code' => $code, 'time' => time()));
        $redis->setex($phone, 15 * 60, $data);
        //return $notice_coed;
    }
     ...
     */
    public function get_notice($phone)
    {
        $redis = new \redis();
        $result = $redis->connect('127.0.0.1', 6379);
        ...
        if ($userlist) {
            ...
            if (time() - $notice_data['time'] > 15 * 60) {
                ...
                if ($notice_data['code'] == $code) {
                    echo json_encode(array('msg' => '验证成功', 'code' => '0', 'status' => 'success'));
                } else {
    ...
    {
        //header("loaction:href");
        //echo date("Y-m-d H:i:s",1494609480);die;
        header("location:http://www.oxygenhoop.com");
        //echo '请求错误';
        //echo getcwd();die;
        //die;
        /*echo time();
        //file_put_contents('./a.txt','fsffsdf');
        $str = '约炮ee约炮4444，加QQ啪啪啪54grfg4QQ群SEX约炮';
        echo filter_name($str);
        $amodel = M("Active");
        $list = $amodel->where(array('ordernum' => 'wxc767748b8c5bfa4b1492412370'))->find();
        print_r($list);*/
        //echo 1;die;
        //layout();
        $redis = new \redis();
        $result = $redis->connect('127.0.0.1', 6379);
        $sid = $redis->set("fsdf", 'fdsf');
        //echo $sid;
        //echo $redis->get('fsdf');
        //echo json_encode(array('msg'=>'添加成功','status'=>true,'active_id'=>'fsfd'));
        //dump(phpinfo());
        //echo 1;die;
        //$this->display();
    }
    ...
    public function join_active()
    {
        ...
        if ($userlist) {
            $model = M('Order');
            $amodel = M('Active');
            $alist = $amodel->field('acode,price,member_id,start_time,number,id,is_pay,start_time')->find($info['aid']);
            if ($alist['number'] <= 0) {
    ...
    public function get_tj_list()
    {
        ...
        $lj = "https://" . $_SERVER['HTTP_HOST'] . "/Public/Uploads/";
        $list = $model->alias('a')->
        field('a.id,CONCAT("' . $lj . '",a.pic) logo,FROM_UNIXTIME(start_time,"%Y-%m-%d %H:%i") starttime')
            ->where(array(
                'a.acode' => array('eq', '1'),
                'a.is_nav' => array('eq', '1'),
    ...
    public function get_active_detial($aid)
    {
        $model = M('Active');
        $lj = "https://" . $_SERVER['HTTP_HOST'] . "/Public/Uploads/";
        $list = $model->alias('a')->
